...
vi.mock("@/app/sun-kudos/_components/recipient-autocomplete", () => ({
  RecipientAutocomplete: ({ value, onChange, inputId }: { value: string; onChange: (v: string) => void; inputId?: string }) => (
    <input id={inputId} data-testid="RecipientAutocomplete" value={value} onChange={(e) => onChange(e.target.value)} />
  ),
}));
...
describe("KudosWriteDialog", () => {
  ...
  it("renders the dialog with title, recipient, editor, and submit button", () => {
    render(<KudosWriteDialog action={action} onSuccess={onSuccess} onClose={onClose} />);
  ...
  it("enables submit when content has non-whitespace characters", () => {
    render(<KudosWriteDialog action={action} onSuccess={onSuccess} onClose={onClose} />);
    const textarea = screen.getByPlaceholderText("kudos.dialog.content_placeholder");
  ...
  it("captures content typing", () => {
    render(<KudosWriteDialog action={action} onSuccess={onSuccess} onClose={onClose} />);
    const textarea = screen.getByPlaceholderText("kudos.dialog.content_placeholder") as HTMLTextAreaElement;
  ...
  it("captures honor-title typing", () => {
    render(<KudosWriteDialog action={action} onSuccess={onSuccess} onClose={onClose} />);
    const input = screen.getByPlaceholderText("kudos.dialog.honor_placeholder") as HTMLInputElement;
  ...
  it("bubbles success via onSuccess once the action returns ok=true", async () => {
    ...
    render(
      <KudosWriteDialog action={successAction} onSuccess={onSuccess} onClose={onClose} />,
    );
  ...
  it("renders the inline error message when the action returns ok=false with an error", async () => {
    ...
    render(
      <KudosWriteDialog action={errorAction} onSuccess={onSuccess} onClose={onClose} />,
    );

    fireEvent.change(screen.getByPlaceholderText("kudos.dialog.content_placeholder"), {
      target: { value: "hi" },
    });
    fireEvent.click(screen.getByText("common.send"));
